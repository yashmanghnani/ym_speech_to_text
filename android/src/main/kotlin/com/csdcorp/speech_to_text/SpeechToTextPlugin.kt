...
public class SpeechToTextPlugin :
    ...
    companion object {
        private const val PLUGIN_TAG = "SpeechToTextPlugin"
    }
...
//            var recognizerIntent = Intent(RecognizerIntent.ACTION_GET_LANGUAGE_DETAILS)
    ...
    private fun optionallyStopBluetooth() {
        ...
        if (lactive != null && lbt != null) {
            ...
            // Check MODIFY_PHONE_STATE permission before calling stopVoiceRecognition
            if (ContextCompat.checkSelfPermission(
                ...
                try {
                    ...
                    Log.e(PLUGIN_TAG, "stopVoiceRecognition failed due to missing permission", e)
                }
            } else {
                Log.w(PLUGIN_TAG, "stopVoiceRecognition skipped: MODIFY_PHONE_STATE permission not granted")
            }
    ...
    private fun setupBluetooth() {
        ...
        val localContext = pluginContext ?: return
        
        // Check for BLUETOOTH_CONNECT permission before accessing bluetooth features
        if (ContextCompat.checkSelfPermission(
                localContext,
                Manifest.permission.BLUETOOTH_CONNECT
            ) != PackageManager.PERMISSION_GRANTED
        ) {
            debugLog("BLUETOOTH_CONNECT permission not granted, skipping bluetooth setup")
            return
        }
        
        try {
            bluetoothAdapter = BluetoothAdapter.getDefaultAdapter()
            // Check permission again before calling getBondedDevices()
            @SuppressLint("MissingPermission")
            ...
            Log.e(logTag, "SecurityException during bluetooth setup", e)
